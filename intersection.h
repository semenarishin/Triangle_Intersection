...
public:
	...
	~Result() {}
	void add(CGLPoint p, double eps) {
		bool has = false;
		for (int i = 0; i < res.size(); i++)
			if (res[i].is_same(p, eps))
				has = true;
		if (!has)
			res.push_back(p);
	}
...
private:
	...
	bool out_of_plane_trs_intersect(CGLTriangle t1, CGLTriangle t2, CGLSegment* seg, Result<CGLPoint>* res);
};
